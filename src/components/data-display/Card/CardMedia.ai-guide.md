...
3. **Implement with proper TypeScript**:
   ```typescript
   const MyCardMedia = (props: MyCardMediaProps) => {
...
3. **❌ Inline styles without theme**
   ...
   **✅ Theme-based styling**
   ```typescript
   // Correct
   <CardMedia sx={{ color: 'error.main' }} />
   ```

4. **❌ Missing accessibility attributes**
   ```typescript
   // Wrong
   <CardMedia onClick={handler} />
   ```
...
- [ ] Props interface is properly typed with `CardMediaProps`
...
4. **Interaction Testing**:
   ...
   it('handles user interactions', async () => {
     ...
     render(<CardMedia onClick={handleClick} />);
