...
3. **Implement with proper TypeScript**:
   ```typescript
   const MyRadioGroup = (props: MyRadioGroupProps) => {
...
3. **❌ Inline styles without theme**
   ...
   **✅ Theme-based styling**
   ```typescript
   // Correct
   <RadioGroup sx={{ color: 'error.main' }} />
   ```

4. **❌ Missing accessibility attributes**
   ```typescript
   // Wrong
   <RadioGroup onClick={handler} />
   ```
...
- [ ] Props interface is properly typed with `RadioGroupProps`
